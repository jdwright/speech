function [out,outp] = filter_async(filters,source,seg_len,fs,bw,ampq)
% filter asynchronously
% filter changes at fixed rate
samp_rate = 22050;
num_samp = length(source);
seg_num = floor(num_samp/seg_len);
seg_mod = mod(num_samp,seg_len);
seg_len2 = round(seg_len/2);
...
for sampn = 1:seg_len:(seg_num-1)*seg_len+1
    vec = sampn:sampn+seg_len-1;
    midp = sampn+seg_len2;
%    ft = filters(fti(midp),:); % select from filter matrix
    ft = make_filter(fs,bw); % create filter on the fly
    out(vec) = filtfilt(ft(1:45),ft(46:90),source(vec));
end
